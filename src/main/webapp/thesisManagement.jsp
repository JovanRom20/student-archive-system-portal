            ...
            <div class="management-wrapper">
                <div class="management-panel">
                 <div class="controls-header">
                        <form action="thesisManagement.jsp" method="GET" style="margin: 0;">
                        ...
                        <a href="addThesis.jsp" class="btn-add">
                    ...
                    <table>
                        ...
                        <tbody>
                            <%
                                // 1. Catch the search query from the URL
                                ...
                                try (Connection conn = DatabaseConnection.getConnection()) {
                                    ...
                                    // 2. Change the SQL depending on if they are searching or not
                                    if (hasSearch) {
                                        // ILIKE means "Case-Insensitive partial match" in PostgreSQL
                                        sql = "SELECT thesis_id, title, author, submission_date, status " +
                                              "FROM Thesis_Records " +
                                              "WHERE title ILIKE ? OR author ILIKE ? " +
                                              "ORDER BY thesis_id DESC";
                                    } else {
                                        // Standard query for when the page first loads
                                        sql = "SELECT thesis_id, title, author, submission_date, status FROM Thesis_Records ORDER BY thesis_id DESC";
                                    }

                                    try (PreparedStatement ps = conn.prepareStatement(sql)) {
                                        
                                        // 3. If searching, inject the text into the SQL query
                                        if (hasSearch) {
                                            // Adding % around the text means "find this word anywhere"
                                            String wildcardSearch = "%" + searchParam.trim() + "%";
                                            ps.setString(1, wildcardSearch); // For title
                                            ps.setString(2, wildcardSearch); // For author
                                        }
                            ...
                            <%
                                            ...
                                            // 4. Show a friendly message if the search finds nothing!
                                            if (!foundResults) {
                                                out.println("<tr><td colspan='5' style='text-align:center; padding: 30px;'>No thesis records found matching your search.</td></tr>");
                                            }
                                        }
                                    }
                                } catch (SQLException e) {
                                    out.println("<tr><td colspan='5' style='text-align:center;'>Error loading thesis records.</td></tr>");
                                    e.printStackTrace();
                                }
                            %>
                        </tbody>
